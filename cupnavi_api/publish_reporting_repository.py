...
def _owns(conn,account_id,tournament_id):return bool(conn.execute("SELECT 1 FROM tournament_members WHERE tournament_id=? AND account_id=? LIMIT 1",(int(tournament_id),int(account_id))).fetchone())
def _publication_payload(conn,tournament_id):
 ...
 keys=set(t.keys());scheduled=int(conn.execute("SELECT COUNT(*) FROM matches WHERE tournament_id=? AND scheduled_start IS NOT NULL",(int(tournament_id),)).fetchone()[0]);blockers=build_publish_blockers(playoff_model_confirmed=bool(t["playoff_format"]) if "playoff_format" in keys else True,scheduled_matches=scheduled,schedule_dirty=bool(t["schedule_dirty"]) if "schedule_dirty" in keys else False,schedule_errors=())
...
def admin_publication(account_id,tournament_id):
 with get_conn() as conn:return _publication_payload(conn,tournament_id) if _owns(conn,account_id,tournament_id) else None
...
def save_result(account_id,tournament_id,match_id,home_score,away_score,expected_home,expected_away):
 ...
 with get_conn() as conn:
  if not _owns(conn,account_id,tournament_id):return None
  row=conn.execute("SELECT home_score,away_score FROM matches WHERE id=? AND tournament_id=?",(int(match_id),int(tournament_id))).fetchone()
  if not row:return None
  if row["home_score"]!=expected_home or row["away_score"]!=expected_away:raise RuntimeError("Resultatet har ändrats av någon annan. Ladda om innan du sparar igen.")
  conn.execute("UPDATE matches SET home_score=?,away_score=?,status='played' WHERE id=? AND tournament_id=?",(int(home_score),int(away_score),int(match_id),int(tournament_id)));conn.commit();return dict(conn.execute("SELECT id,home_score,away_score,status FROM matches WHERE id=?",(int(match_id),)).fetchone())
