...
FLOW_STEPS = ["Grundsetup", "Lag", "Grupper", "Schema", "Kontroll", "Publicera"]
FLOW_ROUTES = {
    "Grundsetup": "Cupinställningar",
    "Lag": "Lag",
    "Grupper": "Grupper",
...
def render_clickable_planning_flow(st, *, tid: int, current_step: str, navigate_admin_page) -> None:
    """Render the six-step planning journey as real navigation buttons.
...
def render_problem_actions(st, *, tid: int, navigate_admin_page, needs_teams: bool = False,
    ...
    if needs_setup:
        actions.append(("Gå till Grundsetup →", "Cupinställningar"))
    if needs_schedule:
