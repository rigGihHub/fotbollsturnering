...
def test_recovery_actions_apply_and_regenerate():
    ...
    assert "Tillämpa +" in RECOVERY and "och generera om" in RECOVERY
    assert "Ta bort reservationerna och generera om" in RECOVERY
    assert "Lägg till 1 plan och generera om" in RECOVERY
    assert "Nästa bästa lösning visas nedan" in APP

def test_soft_late_preference_not_claimed_as_blocking_solution():
    assert "Detta är en mjuk prioritering och blockerar inte i sig schemaläggningen" in RECOVERY
