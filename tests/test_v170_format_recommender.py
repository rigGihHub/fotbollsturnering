...
def test_setup_recommendation_requires_explicit_acceptance():
    assert "### 3. Rekommenderat tävlingsformat" in SETUP
    assert "Inget ändras förrän du accepterar" in SETUP
    assert "Använd rekommenderat format" in SETUP
...
def test_admin_can_review_saved_recommendation():
    assert "Tävlingsprofil och rekommendation" in APP
