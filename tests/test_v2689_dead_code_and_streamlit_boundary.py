...
def test_release_version_is_synchronized():
    assert '"version": "2.6.93"' in (ROOT / "frontend-next/package.json").read_text(encoding="utf-8")
    assert '"version": "2.6.93"' in (ROOT / "frontend-next/package-lock.json").read_text(encoding="utf-8")
    assert 'const APP_VERSION = "2.6.93";' in (ROOT / "frontend-next/src/app/layout.tsx").read_text(encoding="utf-8")
    assert 'const CACHE="cupnavi-next-v2693";' in (ROOT / "frontend-next/public/sw.js").read_text(encoding="utf-8")
