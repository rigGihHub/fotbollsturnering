...
def test_open_setup_uses_callback():
    start=APP.index('with st.expander("Cupadministration", expanded=False):')
    end=APP.index('st.divider()', start)
    block=APP[start:end]
    assert 'def _open_sidebar_cup_setup()' in block
    assert 'on_click=_open_sidebar_cup_setup' in block
