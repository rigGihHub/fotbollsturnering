...
def test_release_2693_is_synchronized_and_ci_uses_maintained_gate():
    assert '"version": "2.6.94"' in read("frontend-next/package.json")
    assert '"version": "2.6.94"' in read("frontend-next/package-lock.json")
    assert 'APP_VERSION = "2.6.94"' in read("frontend-next/src/app/layout.tsx")
    assert 'cupnavi-next-v2694' in read("frontend-next/public/sw.js")
