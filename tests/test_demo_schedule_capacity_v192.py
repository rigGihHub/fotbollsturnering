...
def test_demo_schedule_has_deterministic_capacity_fallback():
    ...
    assert "pitch_count=CASE WHEN pitch_count < 4 THEN 4 ELSE pitch_count END" in APP
    assert "first_match_time='08:00'" in APP
    assert "latest_kickoff_time='22:00'" in APP
