...
function matchNeedsReview(match:ImportedMatch) {
  return !match.time?.trim() || !match.home_team?.trim() || !match.away_team?.trim() || !match.venue?.trim();
}

function matchLabel(match:ImportedMatch, index:number) {
  const teams = [match.home_team?.trim(), match.away_team?.trim()].filter(Boolean).join(" – ");
  return teams || `Match ${index + 1}`;
}

export default function CupCreateLauncherV3() {
  ...
  const matchesToReview = useMemo(() => proposalMatches.reduce<number[]>((rows,match,index) => { if (matchNeedsReview(match)) rows.push(index); return rows; }, []), [proposalMatches]);
  ...
  function resetDialog() {
    ...
    setProposal(null); setImportSchedule(false); setImportStep(0); setExpandedMatch(null); setError("");
  }
  ...
  async function analyzeFiles() {
    ...
    setBusy(true); setError(""); setProposal(null); setExpandedMatch(null);
    try {
      ...
      const firstProblem = (result.matches || []).findIndex(matchNeedsReview);
      setExpandedMatch(firstProblem >= 0 ? firstProblem : null);
    } catch (err) {
  ...
  const stepGrid = { display:"grid", gridTemplateColumns:"repeat(4,minmax(0,1fr))", gap:6, margin:"0 0 16px" };
      ...
      <section className="cup-create-dialog" role="dialog" aria-modal="true" aria-labelledby="cup-create-title" style={modalStyle}>
        ...
        {mode === "manual" ? <form onSubmit={createCup}>
          ...
          {!proposal ? <>
            ...
            <div style={stepGrid}>{CUP_IMPORT_STEPS.map((step,index) => <button key={step.id} type="button" className={step.id===importStep?"":"is-secondary"} onClick={() => setImportStep(step.id)} style={{padding:"8px 6px",fontSize:12}}>{step.id < importStep ? "✓ " : `${index+1}. `}{step.label}</button>)}</div>
            ...
            {importStep === 0 && <div>
              ...
              {!!(proposal.rules||[]).length && <details style={{marginTop:12}}><summary style={{fontWeight:900,cursor:"pointer"}}>{(proposal.rules||[]).length} regler hittade</summary><ul>{proposal.rules!.map((rule,index) => <li key={index}>{rule}</li>)}</ul></details>}
            </div>}
            ...
            {importStep === 2 && <div><h3>Matcher</h3>
              <p style={{fontSize:13,marginBottom:10}}>{matchesToReview.length ? `${matchesToReview.length} matcher behöver kontrolleras. Övriga är hopfällda för snabb överblick.` : "Alla matcher ser kompletta ut. Öppna bara en match om du vill ändra något."}</p>
              <div className="cup-import-match-list">{proposalMatches.map((match,index) => {
                const needsReview = matchNeedsReview(match);
                const expanded = expandedMatch === index;
                return <section key={index} className={`cup-import-match-card${needsReview?" needs-review":""}${expanded?" is-open":""}`}>
                  <button type="button" className="cup-import-match-summary" onClick={() => setExpandedMatch(expanded ? null : index)} aria-expanded={expanded}>
                    <span className="cup-import-match-time">{match.time?.trim() || "Tid?"}</span>
                    <span className="cup-import-match-teams"><strong>{matchLabel(match,index)}</strong><small>{[match.group_name,match.venue].filter(Boolean).join(" · ") || "Grupp/plan saknas"}</small></span>
                    <span className={`cup-import-match-status ${needsReview?"warn":"ok"}`}>{needsReview?"Kontrollera":"✓"}</span>
                  </button>
                  {expanded && <div className="cup-import-match-editor">
                    <label>Tid<input value={match.time||""} onChange={e => updateImportedMatch(index,"time",e.target.value)} /></label>
                    <label>Grupp<input value={match.group_name||""} onChange={e => updateImportedMatch(index,"group_name",e.target.value)} /></label>
                    <label>Hemma<input value={match.home_team||""} onChange={e => updateImportedMatch(index,"home_team",e.target.value)} /></label>
                    <label>Borta<input value={match.away_team||""} onChange={e => updateImportedMatch(index,"away_team",e.target.value)} /></label>
                    <label>Plan<input value={match.venue||""} onChange={e => updateImportedMatch(index,"venue",e.target.value)} /></label>
                    <button type="button" className="is-secondary cup-import-match-done" onClick={() => setExpandedMatch(null)}>Klar</button>
                  </div>}
                </section>;
              })}</div>
              <label className="cup-import-schedule-toggle"><input type="checkbox" checked={importSchedule} onChange={e => setImportSchedule(e.target.checked)} /> <span><strong>Använd matchprogrammet</strong><small>Importera det granskade schemat till cupen</small></span></label>
              {!!(proposal.playoff_matches||[]).length && <p><strong>Slutspel hittat:</strong> {(proposal.playoff_matches||[]).length} matcher/källor.</p>}
            </div>}
            ...
            {importStep === 4 && <div><h3>Slutkontroll</h3><p><strong>{name || "Cup utan namn"}</strong>{startDate?` · ${startDate}`:""}</p>
              ...
              {!!matchesToReview.length && <div className="cup-create-error"><strong>{matchesToReview.length} matcher är fortfarande ofullständiga.</strong><p style={{margin:"4px 0 0"}}>Gå tillbaka till Matcher och kontrollera tid, lag och plan innan du importerar schemat.</p></div>}
            ...
            <div className="cup-create-actions" style={{position:"sticky",bottom:0,zIndex:3,background:"var(--paper-2, #fbfaf5)",paddingTop:10}}>
              {importStep === 0 ? <button type="button" className="is-secondary" disabled={busy} onClick={() => { setProposal(null); setImportSchedule(false); setExpandedMatch(null); setError(""); }}>← Byt filer</button> : <button type="button" className="is-secondary" disabled={busy} onClick={() => setImportStep(previousCupImportStep(importStep))}>← Tillbaka</button>}
              {importStep !== 4 ? <button type="button" disabled={busy || (importStep===0 && !name.trim())} onClick={() => setImportStep(nextCupImportStep(importStep))}>Nästa →</button> : <button type="submit" disabled={busy || !name.trim() || (importSchedule && matchesToReview.length > 0)}>{busy?"Importerar…":"✓ Skapa cup"}</button>}
            </div>
          </form>}
        </div>}
      </section>
