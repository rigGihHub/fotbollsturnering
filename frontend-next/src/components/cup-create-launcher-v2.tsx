...
export default function CupCreateLauncherV2() {
        ...
        {mode === "manual" ? <form onSubmit={createCup}>
          ...
          {!proposal ? <>
            ...
            <div className="cup-create-actions" style={{position:"sticky",bottom:0,zIndex:3,background:"var(--paper-2, #fbfaf5)",paddingTop:10}}>
              {importStep === 0 ? <button type="button" className="is-secondary" disabled={busy} onClick={() => { setProposal(null); setImportSchedule(false); setError(""); }}>← Byt filer</button> : <button type="button" className="is-secondary" disabled={busy} onClick={() => setImportStep(previousCupImportStep(importStep))}>← Tillbaka</button>}
              {importStep !== 4 ? <button type="button" disabled={busy || (importStep===0 && !name.trim())} onClick={() => setImportStep(nextCupImportStep(importStep))}>Nästa →</button> : <button type="submit" disabled={busy || !name.trim()}>{busy?"Importerar…":"✓ Skapa cup"}</button>}
            </div>
          </form>}
        </div>}
      </section>
