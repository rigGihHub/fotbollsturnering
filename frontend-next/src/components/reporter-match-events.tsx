...
export default function ReporterMatchEvents({token,cupId,online,queueSignal,enabled,onAuthError}:{token:string;cupId:number;online:boolean;queueSignal:number;enabled:ReporterEventSettings;onAuthError?:()=>void}){
 ...
 return <section className="admin-panel reporter-events" id="reporter-events">
  ...
  <div className="reporter-events__head"><div><h2>{[canGoals&&"Målskyttar",canAssists&&"assist",canCards&&"kort"].filter(Boolean).join(" & ")}</h2><p>Välj en färdigspelad match. Varje tryck sparas direkt eller läggs tryggt i offlinekön.</p></div><span className="admin-lock">STEG 2</span></div>
  ...
  {selected&&detail&&<><div className="reporter-events__selected"><span>VALD MATCH</span><strong>{selected.home_team_name} {selected.home_score}–{selected.away_score} {selected.away_team_name}</strong><small>{selected.stage||"Match"}</small>{locked&&<small>Matchen är slutmarkerad. Endast administratören kan ändra händelser.</small>}</div><div className="reporter-events__teams">{detail.teams.map(team=><section key={team.team_id} className="reporter-event-team"><header><div><span>{team.side==="home"?"HEMMA":"BORTA"}</span><h3>{team.team_name}</h3></div><strong>{canGoals?`${team.registered_goals}/${team.team_score} mål kopplade`:""}</strong></header>{team.players.length?<div className="reporter-player-list">{team.players.map(player=><article key={player.id}><div><strong>{player.player_number!=null?`${player.player_number}. `:""}{player.name}</strong><small>{canGoals?`${player.goals} mål`:""}{canAssists&&detail.enabled.assists?` · ${player.assists} assist`:""}{canCards&&detail.enabled.cards?` · ${player.yellow_cards} gula · ${player.red_cards} röda`:""}</small></div><div className="reporter-player-actions">{canGoals&&<Counter label="Mål" value={player.goals} disabled={locked||!!busyKey} minus={()=>void change(player,"goals",-1)} plus={()=>void change(player,"goals",1)}/>}{canAssists&&detail.enabled.assists&&<Counter label="Assist" value={player.assists} disabled={locked||!!busyKey} minus={()=>void change(player,"assists",-1)} plus={()=>void change(player,"assists",1)}/>} {canCards&&detail.enabled.cards&&<><Counter label="Gult" value={player.yellow_cards} disabled={locked||!!busyKey} minus={()=>void change(player,"yellow_cards",-1)} plus={()=>void change(player,"yellow_cards",1)}/><Counter label="Rött" value={player.red_cards} disabled={locked||!!busyKey} minus={()=>void change(player,"red_cards",-1)} plus={()=>void change(player,"red_cards",1)}/></>}</div></article>)}</div>:<div className="reporter-events__empty"><strong>Trupp saknas</strong><span>Lägg in spelarna i lagets trupp för att kunna koppla mål och kort.</span></div>}{canGoals&&team.registered_goals<team.team_score&&<p className="reporter-events__warning">{team.team_score-team.registered_goals} mål saknar spelarkoppling, exempelvis självmål eller okänd målskytt.</p>}</section>)}</div></>}
 </section>;
}
