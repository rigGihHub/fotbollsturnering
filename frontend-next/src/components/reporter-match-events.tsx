...
export default function ReporterMatchEvents({token,onAuthError}:{token:string;onAuthError?:()=>void}){
 ...
 return <section className="admin-panel reporter-events" id="reporter-events">
  ...
  <div className="reporter-events__head"><div><h2>Målskyttar, assist & kort</h2><p>Välj en färdigspelad match. Varje ändring sparas direkt och kontrolleras mot slutresultatet.</p></div><span className="admin-lock">STEG 2</span></div>
...
function Counter({label,value,disabled,minus,plus}:{label:string;value:number;disabled:boolean;minus:()=>void;plus:()=>void}){
 return <span className="reporter-counter"><small>{label}</small><button type="button" aria-label={`Minska ${label}`} disabled={disabled||value<=0} onClick={minus}>−</button><strong>{value}</strong><button type="button" aria-label={`Öka ${label}`} disabled={disabled} onClick={plus}>+</button></span>;
}
