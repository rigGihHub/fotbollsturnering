    ...
    <section className="texttv texttv--standings" aria-labelledby={`table-${name}`}>
      ...
      <div className="texttv__scroll">
        <table>
          ...
          <tbody>{rows.map((row)=><tr className={rowTone(row)} key={row.team_id}><td>{row.position}</td><td><span className="standings-team-name">{row.Lag}</span><span className="standings-team-meta">S {row.S} · MS {row.MS}</span></td><td>{row.S}</td><td>{row.V}</td><td>{row.O}</td><td>{row.F}</td><td>{row.MS}</td><td><strong>{row.P}</strong></td></tr>)}</tbody>
        </table>
