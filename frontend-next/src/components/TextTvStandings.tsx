...
export function TextTvStandings({ name, rows, destinations=[], positionDestinations={} }: { name: string; rows: StandingRow[]; destinations?:string[]; positionDestinations?:Record<number,number> }) {
  ...
  const rowTone=(row:StandingRow,index:number)=>positionDestinations[row.position]!==undefined?tone(positionDestinations[row.position]):tone(index);
  return (
    <section className="texttv texttv--standings" aria-labelledby={`table-${name}`}>
      ...
      <div className="texttv__scroll">
        <table>
          ...
          <tbody>{rows.map((row,index)=><tr className={rowTone(row,index)} key={row.team_id}><td>{row.position}</td><td>{row.Lag}</td><td>{row.S}</td><td>{row.V}</td><td>{row.O}</td><td>{row.F}</td><td>{row.MS}</td><td><strong>{row.P}</strong></td></tr>)}</tbody>
        </table>
