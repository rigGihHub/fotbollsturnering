...
const publicKitModeFor=(info?:CupInfo|null):PublicKitMode=>info?.show_public_kits===false||info?.show_public_kits===0?"none":info?.show_public_away_kits===false||info?.show_public_away_kits===0?"home":"both";
...
function cleanCupInfo(value:CupInfo):CupInfo {
  const weatherConfigured = value.show_public_weather_configured === true || value.show_public_weather_configured === 1;
  return {
    ...value,
    name:value.name || "",
    ...
    show_public_weather:weatherConfigured ? value.show_public_weather : true,
    show_public_weather_configured:weatherConfigured,
  };
}

const publicWeatherEnabled = (cupinfo:CupInfo) => cupinfo.show_public_weather_configured === false || cupinfo.show_public_weather_configured === 0
  ? true
  : cupinfo.show_public_weather !== false && cupinfo.show_public_weather !== 0;
...
export default function AdminWorkspace({verifiedSession=null,children=null}:{verifiedSession?:(SessionPayload & {token:string})|null;children?:ReactNode}) {
  ...
  const [kitBusy,setKitBusy] = useState<"kit"|"logo"|null>(null);
  ...
  async function searchAssets(focus:"kit"|"logo",candidate?:KitCandidate) {
    ...
    if(focus==="kit"&&kitMode==="none"){setMessage("Matchställ är avstängda i publik visning. Välj hemmatröja eller hemma- och bortatröja för att söka.");return;}
    setAssetFocus(focus);
  ...
  function applyKitSuggestion(){
    ...
    setTeamDraft(current=>({...current,
      ...
      ...(publicKitModeFor(cupinfo)==="both"&&kitSuggestion.away_verified?{secondary_color:kitSuggestion.away_color_1,away_color_2:kitSuggestion.away_color_2,away_pattern:kitSuggestion.away_pattern}:{}),
      ...
      {activeStep==="teams" && <section className="admin-panel admin-teams" id="teams">
        ...
        {(!teams.length || teamFormOpen) && <form ref={teamFormRef} onSubmit={saveTeam} className="admin-team-editor">
          ...
          <section className="admin-kit-search" aria-label="Sök lagets matchställ och klubbmärke">
            ...
            <div className="admin-asset-search-actions"><button type="button" disabled={kitBusy==="kit"||!teamDraft.name.trim()} aria-busy={kitBusy==="kit"} onClick={()=>void searchAssets("kit")}>{kitBusy==="kit"?"Söker matchställ…":"Sök matchställ"}</button><button type="button" disabled={kitBusy==="logo"||!teamDraft.name.trim()} aria-busy={kitBusy==="logo"} onClick={()=>void searchAssets("logo")}>{kitBusy==="logo"?"Söker klubbmärke…":"Sök klubbmärke"}</button></div>
          </section>
          {kitSuggestion&&!kitSuggestion.found&&!kitSuggestion.home_verified&&!kitSuggestion.away_verified&&kitSuggestion.identity_status!=="exact"&&<aside className="admin-kit-help"><strong>Hittade vi inte rätt tröja?</strong><ol><li>Skriv klubbens <b>ort</b> i Sökledtråd och sök igen.</li><li>Om namnet är vanligt: klistra in klubbens <b>officiella webbplats</b> eller offentliga Instagram/Facebook-sida.</li><li>Välj rätt klubb om CupNavi visar flera kandidater.</li><li>Finns ingen säker källa: välj tröjfärg och mönster manuellt ovan. CupNavi sparar aldrig en osäker träff automatiskt.</li></ol><small>Tips: skriv hellre “Stångebro United Linköping” eller klubbens webbadress än bara “Stångebro”.</small></aside>}{kitSuggestion&&<section className="admin-kit-result">
            {kitSuggestion.candidate_matches?.length>0&&!kitSuggestion.found&&!kitSuggestion.logo_verified?<><strong>Vilken klubb är rätt?</strong><p>Flera möjliga klubbar hittades. Välj rätt identitet innan uppgifter används.</p><div className="admin-kit-candidates">{kitSuggestion.candidate_matches.map(candidate=><button type="button" key={candidate.source_url} onClick={()=>void searchAssets(assetFocus,candidate)}><b>{candidate.name}</b><span>{[candidate.location,candidate.country].filter(Boolean).join(" · ")}</span><small>{candidate.reason}</small></button>)}</div></>:<><div className="admin-kit-result__head"><div><span>KLUBBIDENTITET · {kitSuggestion.identity_status==="exact"?"VERIFIERAD":kitSuggestion.identity_status==="likely"?"TROLIG":"OSÄKER"}</span><strong>{kitSuggestion.club_match||teamDraft.name}</strong><small>Hemma · {kitSuggestion.home_verified?"verifierat":"kontrollera"}{publicKitModeFor(cupinfo)==="both"&&<> &nbsp; Borta · {kitSuggestion.away_verified?"verifierat":"kontrollera"}</>} &nbsp; Märke · {kitSuggestion.logo_verified?"verifierat":"saknas"}</small></div><button type="button" disabled={!kitSuggestion.home_verified&&!kitSuggestion.away_verified&&!kitSuggestion.logo_verified} onClick={applyKitSuggestion}>Använd verifierade uppgifter</button></div><div className="admin-kit-options">{assetFocus!=="logo"&&<><div><i className="admin-kit-swatch" style={{background:kitBackground(kitSuggestion.home_pattern,kitSuggestion.home_color_1,kitSuggestion.home_color_2)} as CSSProperties}/><span><b>Hemma · {kitSuggestion.home_pattern}</b><small>{kitSuggestion.home_verified?kitSuggestion.home_evidence:"Inte verifierat"}</small></span></div>{publicKitModeFor(cupinfo)==="both"&&<div><i className="admin-kit-swatch" style={{background:kitBackground(kitSuggestion.away_pattern,kitSuggestion.away_color_1,kitSuggestion.away_color_2)} as CSSProperties}/><span><b>Borta · {kitSuggestion.away_pattern}</b><small>{kitSuggestion.away_verified?kitSuggestion.away_evidence:"Inte verifierat"}</small></span></div>}</>}{kitSuggestion.logo_verified?<div><TeamLogo team={{...teamDraft,id:editingTeam||0,tournament_id:cupId||0,name:kitSuggestion.club_match||teamDraft.name,logo_url:kitSuggestion.logo_url} as Team}/><span><b>Klubbmärke</b><small>Verifierat mot klubbkällan. Om bilden inte kan laddas visas klubbens initialer i stället.</small></span></div>:assetFocus==="logo"&&<div><span><b>Inget verifierat klubbmärke</b><small>Förtydliga klubbens ort eller officiella webbplats.</small></span></div>}</div><details className="admin-kit-evidence"><summary>Visa varför CupNavi föreslår detta</summary><div className="admin-kit-evidence__grid">{kitSuggestion.home_sources?.length>0&&<section><strong>Hemma</strong><small>{kitSuggestion.home_evidence||"Verifierad källa"}</small>{kitSuggestion.home_sources.map(url=><span key={url} className="admin-kit-evidence__source">{directImageUrl(url)&&<img src={directImageUrl(url)} alt="" loading="lazy" referrerPolicy="no-referrer"/>}<a href={url} target="_blank" rel="noreferrer">Öppna bild/källa ↗</a></span>)}</section>}{publicKitModeFor(cupinfo)==="both"&&kitSuggestion.away_sources?.length>0&&<section><strong>Borta</strong><small>{kitSuggestion.away_evidence||"Verifierad källa"}</small>{kitSuggestion.away_sources.map(url=><a key={url} href={url} target="_blank" rel="noreferrer">Öppna bild/källa ↗</a>)}</section>}{kitSuggestion.logo_source_url&&<section><strong>Klubbmärke</strong><small>Identitetskälla för klubbmärket.</small><a href={kitSuggestion.logo_source_url} target="_blank" rel="noreferrer">Öppna källa ↗</a></section>}</div></details></>}
          </section>}
