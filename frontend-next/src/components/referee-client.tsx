...
import {REFEREE_QUEUE_EVENT,completeRefereeMutation,conflictRefereeCount,isNetworkError,pendingRefereeCount,readRefereeQueue,updateRefereeMutation,upsertRefereeMutation} from "../lib/referee-offline";
...
const isAuthFailure=(error:unknown)=>/(401|session|domarinloggning|ogiltig|gått ut|authentication)/i.test(error instanceof Error?error.message:String(error));
...
export default function RefereeClient(){
 ...
 useEffect(()=>{
  const refresh=()=>{setOnline(navigator.onLine);setPending(pendingRefereeCount(cupInfo?.id));setConflicts(conflictRefereeCount(cupInfo?.id))};refresh();
  window.addEventListener("online",refresh);window.addEventListener("offline",refresh);window.addEventListener(REFEREE_QUEUE_EVENT,refresh);
  return()=>{window.removeEventListener("online",refresh);window.removeEventListener("offline",refresh);window.removeEventListener(REFEREE_QUEUE_EVENT,refresh)}
 },[cupInfo?.id]);
 useEffect(()=>{
  ...
  const cached=readCache(),cacheMatchesLink=!queryCup||queryCup===cached?.cup.public_slug||queryCup===String(cached?.cup.id),cacheMatchesReferee=!queryReferee||queryReferee.replace(/\D/g,"")===String(cached?.referee.id);
  if(cached&&cacheMatchesLink&&cacheMatchesReferee){setCupInfo(cached.cup);setReferee(cached.referee);setMatches(cached.matches)}
  void load(stored).catch(reason=>{if(isAuthFailure(reason)){logout();return}if(cached&&cacheMatchesLink&&cacheMatchesReferee){setMessage(navigator.onLine?"Sparad domarvy visas medan CupNavi återansluter.":"Offline: dina senast hämtade matcher visas.");return}logout()});
 },[load,logout]);
 const flushResults=useCallback(async()=>{
  if(!token||!cupInfo||!navigator.onLine||syncing)return;const queued=readRefereeQueue().filter(item=>item.cupId===cupInfo.id&&item.state!=="conflict").sort((a,b)=>a.createdAt-b.createdAt);if(!queued.length)return;
  setSyncing(true);
  try{
   const assignments=await call<AssignmentPayload>("/api/referee/assignments",token);let serverMatches=assignments.matches||[];
   for(const mutation of queued){
    const current=serverMatches.find(item=>item.id===mutation.matchId);if(!current){updateRefereeMutation(mutation.id,{state:"conflict"});continue}
    ...
    if(desired){completeRefereeMutation(mutation);continue}
    ...
    if(!unchanged){updateRefereeMutation(mutation.id,{state:"conflict"});setError("Ett offline-resultat krockar med ett nyare serverresultat och har inte skrivits över.");continue}
    await call(`/api/referee/assignments/matches/${mutation.matchId}`,token,{method:"PUT",body:JSON.stringify(mutation.payload)});completeRefereeMutation(mutation);
    serverMatches=serverMatches.map(item=>item.id===mutation.matchId?{...item,home_score:mutation.payload.home_score,away_score:mutation.payload.away_score,home_penalties:mutation.payload.home_penalties,away_penalties:mutation.payload.away_penalties,played:true}:item);
   }
  ...
  finally{setSyncing(false);setPending(pendingRefereeCount(cupInfo.id));setConflicts(conflictRefereeCount(cupInfo.id))}
 },[cupInfo,load,syncing,token]);
 useEffect(()=>{if(!online||pending===0)return;const retry=window.setTimeout(()=>void flushResults(),500);return()=>window.clearTimeout(retry)},[flushResults,online,pending]);
 ...
 function optimisticResult(match:Match,payload:{home_score:number;away_score:number;home_penalties:number|null;away_penalties:number|null}){
  setMatches(current=>{const next=current.map(item=>item.id===match.id?{...item,...payload,played:true}:item);if(cupInfo&&referee)remember(cupInfo,referee,next);return next});
 }
 function save(match:Match,home:string,away:string,homePenalties:string,awayPenalties:string){
  if(!token||!cupInfo||!referee)return;setError("");setMessage("");
  ...
  upsertRefereeMutation({id:`referee-result-${cupInfo.id}-${match.id}`,cupId:cupInfo.id,matchId:match.id,createdAt:Date.now(),state:"queued",payload});
  optimisticResult(match,payload);setMessage(navigator.onLine?"Registrerat – synkroniserar med servern.":"Sparat lokalt. Resultatet skickas när nätet är tillbaka och stäms av mot servern.");
 }
 const nextMatch=useMemo(()=>matches.find(match=>!match.played)||matches[0]||null,[matches]);
 const pendingMatches=useMemo(()=>new Set(readRefereeQueue().filter(item=>item.cupId===cupInfo?.id&&item.state!=="conflict").map(item=>item.matchId)),[cupInfo?.id,pending,matches]);
 ...
 return <main className="reporter-page referee-page">
  <div className={`reporter-network is-${online?syncing?"syncing":"online":"offline"}`} role="status" aria-live="polite"><span aria-hidden="true"/><strong>{online?syncing?"Synkroniserar":"Online":"Offline"}</strong><small>{conflicts?`${conflicts} stoppad konflikt kräver kontroll`:pending?`${pending} resultat väntar`:online?"Alla resultat är synkroniserade":"Resultat sparas på mobilen"}</small>{online&&pending>0&&<button type="button" onClick={()=>void flushResults()} disabled={syncing}>Synka nu</button>}</div>
  ...
  <section className="admin-panel reporter-results"><div className="admin-panel__top"><span>DOMARE / RESULTAT</span><strong>{matches.length} MATCHER</strong></div><div className="reporter-results__head"><div><h2>Mina matcher</h2><p>Rapportera slutresultat för matcher du är tilldelad.</p></div><span className="admin-lock">DOMARE</span></div><div className="reporter-match-list">{matches.map(match=><RefereeMatch key={match.id} match={match} busy={busy} pending={pendingMatches.has(match.id)} save={save}/>)}</div>{matches.length===0&&<div className="admin-empty"><strong>Inga matcher tilldelade</strong><span>Kontakta arrangören om du saknar matcher.</span></div>}</section>
 </main>;
}

function RefereeMatch({match,busy,pending,save}:{match:Match;busy:boolean;pending:boolean;save:(match:Match,h:string,a:string,hp:string,ap:string)=>void}){
 ...
 return <article className={`reporter-match${match.played?" is-saved":""}${pending?" is-pending":""}`}><div className="reporter-match__meta"><span>{match.scheduled_start?.replace("T"," ")||"Ej schemalagd"}</span><span>{match.pitch_number?`Plan ${match.pitch_number}`:"Plan saknas"}</span>{pending?<span className="reporter-match__pending">Väntar på nät</span>:match.played&&<span className="reporter-match__saved">Sparad</span>}</div><div className="reporter-match__body"><div className="reporter-match__teams"><strong>{match.home_team}</strong><span>mot</span><strong>{match.away_team}</strong></div><div className="reporter-score"><label><span>{match.home_team}</span><input aria-label={`Mål för ${match.home_team}`} inputMode="numeric" type="number" min="0" value={h} onChange={event=>setH(event.target.value)}/></label><b>-</b><label><span>{match.away_team}</span><input aria-label={`Mål för ${match.away_team}`} inputMode="numeric" type="number" min="0" value={a} onChange={event=>setA(event.target.value)}/></label>{tied&&<div className="reporter-penalties"><label>Straffar hemma<input inputMode="numeric" type="number" min="0" value={hp} onChange={event=>setHp(event.target.value)}/></label><label>Straffar borta<input inputMode="numeric" type="number" min="0" value={ap} onChange={event=>setAp(event.target.value)}/></label></div>}<button type="button" disabled={busy||h===""||a===""||(tied&&(hp===""||ap===""))} onClick={()=>save(match,h,a,hp,ap)}>{pending?"Uppdatera lokalt":match.played?"Uppdatera resultat":"Spara resultat"}</button></div></div></article>;
}
