...
export function PublicCupView({ publicKey, initialCup, initialStandings, reporterReturn=false }:{publicKey:string;initialCup:CupSnapshot;initialStandings:StandingsGroup[];reporterReturn?:boolean}){
  ...
  useEffect(()=>{let busy=false;const refresh=async()=>{if(busy||document.visibilityState!=="visible")return;busy=true;try{const freshCup=await getCup(publicKey);setUnavailable(false);setRefreshProblem(false);setCup(normalizeCup(freshCup));if(tab==="table"&&showTables){try{const freshStandings=await getStandings(publicKey);setStandings(Array.isArray(freshStandings.groups)?freshStandings.groups:[]);setStandingsLoaded(true)}catch{}}if(tab==="stats"&&statsEnabled){try{setStatistics(await getStatistics(publicKey))}catch{}}}catch(error){if(error instanceof CupNaviApiError&&error.status===404)setUnavailable(true);else setRefreshProblem(true)}finally{busy=false}};const timer=window.setInterval(refresh,30000);const onVisibility=()=>{if(document.visibilityState==="visible")void refresh()};document.addEventListener("visibilitychange",onVisibility);return()=>{window.clearInterval(timer);document.removeEventListener("visibilitychange",onVisibility)}},[publicKey,showTables,tab,statsEnabled]);
  ...
  return <main className="page-shell page-shell--matchday page-shell--public-v3">
    ...
    {tab==="info"&&<section className="public-info-v3"><div className="public-info-hero"><div><span>Cupinfo</span><h2>Allt inför cupdagen</h2><p>Regler, tider och praktisk information samlat på ett ställe.</p></div></div><div className="public-info-grid">
      ...
      <article className="public-info-card"><span className="public-info-card__eyebrow">Arrangör & plats</span><h3>{cup.tournament.organizer||"Cupinformation"}</h3><p>{cup.tournament.public_information||"Ingen extra information har publicerats."}</p><div className="public-info-details">{cup.tournament.arena_address&&<div><span>Plats</span><b>{cup.tournament.arena_address}</b></div>}{cup.tournament.organizer_phone&&<div><span>Telefon</span><b>{cup.tournament.organizer_phone}</b></div>}{cup.tournament.feedback_email&&<div><span>Kontakt</span><b>{cup.tournament.feedback_email}</b></div>}</div></article>
      {(cup.pitches||[]).length>0&&<article className="public-info-card"><span className="public-info-card__eyebrow">Planer & tider</span><h3>Spelområdet</h3><div className="public-pitch-list">{(cup.pitches||[]).map(pitch=>{const start=pitch.opens_at||pitch.start_time||pitch.available_from;const end=pitch.closes_at||pitch.end_time||pitch.available_to;return <div key={pitch.pitch_number}><span><b>{pitch.name||`Plan ${pitch.pitch_number}`}</b><small>Plan {pitch.pitch_number}</small></span>{(start||end)&&<strong>{start||"–"}{end?`–${end}`:""}</strong>}</div>})}</div></article>}
      {showPlayoffs&&<article className="public-info-card public-info-card--playoff"><span className="public-info-card__eyebrow">Slutspel</span><h3>Vägen efter gruppspelet</h3>{cup.tournament.playoff_format&&<p>{cup.tournament.playoff_format}</p>}<div className="public-playoff-list">{cup.brackets.map((bracket,index)=><div key={bracket.id}><i className={["is-leading","is-neutral","is-playoff","is-sky"][index]||"is-neutral"}/><span>{bracket.name}</span>{bracket.size&&<small>{bracket.size} lag</small>}</div>)}</div>{cup.tournament.bronze_match&&<div className="public-info-note">Bronsmatch spelas.</div>}</article>}
      {cup.venue_points.map(point=><article className="public-info-card" key={point.id}><span className="public-info-card__eyebrow">{(point.kind||"Plats").toUpperCase()}</span><h3>{point.label||"Cupområdet"}</h3><p>{point.detail||"Praktisk information."}</p>{point.url&&<a href={point.url} target="_blank" rel="noreferrer">Öppna karta / länk →</a>}</article>)}
