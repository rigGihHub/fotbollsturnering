...
export function PublicCupView({ publicKey, initialCup, initialStandings, reporterReturn=false }:{publicKey:string;initialCup:CupSnapshot;initialStandings:StandingsGroup[];reporterReturn?:boolean}){
  ...
  return <main className="page-shell page-shell--matchday page-shell--public-v3">
    ...
    {tab==="info"&&<section className="public-info-v3"><div className="public-section-head"><div><span>Cupinfo</span><h2>Bra att veta</h2></div><p>Regler och praktisk information</p></div><div className="public-info-grid">
      <article className="public-info-card public-info-card--rules"><span className="public-info-card__eyebrow">Tävlingsregler</span><h3>{isMatchcamp?"Matchförutsättningar":"Regler för cupen"}</h3><div className="public-rule-list">
        <div><span>Matchtid</span><b>{cup.tournament.halves||1} × {cup.tournament.minutes_per_half||0} min</b></div>
      ...
      <article className="public-info-card"><span className="public-info-card__eyebrow">Arrangör</span><h3>{cup.tournament.organizer||"Cupinformation"}</h3><p>{cup.tournament.public_information||"Ingen extra information har publicerats."}</p><div className="public-info-card__meta">{cup.tournament.arena_address||"Plats kommer"}</div></article>
