...
export function PublicCupView({ publicKey, initialCup, initialStandings, reporterReturn=false }:{publicKey:string;initialCup:CupSnapshot;initialStandings:StandingsGroup[];reporterReturn?:boolean}){
  ...
  const shareCup=async()=>{const url=new URL(`/cup/${encodeURIComponent(publicKey)}`,window.location.origin).toString();try{if(navigator.share){await navigator.share({title:cup.tournament.name,text:`Följ ${cup.tournament.name} i CupNavi`,url});setShareState("Delat");return}await navigator.clipboard.writeText(url);setShareState("Länk kopierad")}catch{setShareState("Dela cupen")}};
