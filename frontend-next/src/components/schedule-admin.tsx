...
export default function ScheduleAdmin({token,cupId}:{token:string;cupId:number}){
  ...
  return <section className="admin-panel admin-teams" id="schedule">
    ...
    {data.unscheduled_count===0&&analysis.ok&&<section className="schedule-ready-card"><span className="schedule-ready-card__check">✓</span><div><small>SCHEMAT ÄR KOMPLETT</small><h3>Alla {data.match_count} matcher har tid och plan</h3><p>Inga krockar hittades. Kontrollera matchlistan nedan och fortsätt sedan till den slutliga publiceringskontrollen.</p></div><a href="#publish">Fortsätt till kontroll & publicering →</a></section>}
    ...
    <div className="admin-team-actions schedule-filters" style={{marginBottom:16}}><button type="button" className={filter==="all"?"is-active":""} aria-pressed={filter==="all"} onClick={()=>setFilter("all")}>Alla</button><button type="button" className={filter==="scheduled"?"is-active":""} aria-pressed={filter==="scheduled"} onClick={()=>setFilter("scheduled")}>Schemalagda</button><button type="button" className={filter==="unscheduled"?"is-active":""} aria-pressed={filter==="unscheduled"} onClick={()=>setFilter("unscheduled")}>Oschemalagda</button></div>
