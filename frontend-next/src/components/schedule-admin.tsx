...
type ProposalUnresolved={match_id:number;reason:"played_without_schedule"|"locked_without_schedule"|"no_feasible_slot"|"round_order_blocked"|"playoff_dependency_blocked"};
...
type ScheduleProposal={
  ...
  capacity:{window_count:number;confirmed_window_count:number;slot_count:number;free_slot_count:number;required_count:number;reason_counts:Record<string,number>;suggestions:string[]};
};
type ApplyResult={
...
function readableKickoff(value?:string|null){
  if(!value)return "Tid saknas";
  const parsed=new Date(value);
  return Number.isNaN(parsed.getTime())?value.replace("T"," · "):new Intl.DateTimeFormat("sv-SE",{dateStyle:"medium",timeStyle:"short"}).format(parsed);
}
...
function unresolvedLabel(reason:ProposalUnresolved["reason"]){
  ...
  if(reason==="playoff_dependency_blocked")return "Tidigare slutspelsmatch måste schemaläggas först";
...
export default function ScheduleAdmin({token,cupId}:{token:string;cupId:number}){
  ...
  async function confirmSchedule(){setBusy(true);setError("");setMessage("");try{const confirmed=await api<SchedulePayload>(`/api/admin/cups/${cupId}/schedule/confirm`,{method:"POST"},token);setData(confirmed);setMessage("Schemat är kontrollerat mot plantider och matchregler. Du kan gå vidare till publicering.");}catch(err){setError(err instanceof Error?err.message:"Schemat kunde inte godkännas.");}finally{setBusy(false);}}
  ...
  return <section className="admin-panel admin-teams" id="schedule">
    ...
    <div className="admin-cupinfo__head"><div><h2>{data.unscheduled_count===0?"Granska matchschemat":"Gör klart matchschemat"}</h2><p>{data.unscheduled_count===0?"Kontrollera att lag, avspark och plan stämmer. Därefter går du vidare till den slutliga publiceringskontrollen.":"CupNavi placerar matcher utifrån planernas öppettider och viloregler. Du granskar alltid förslaget innan det sparas."}</p></div><span className="admin-lock">STEG 6 AV 7</span></div>
    ...
    <div className="admin-dashboard-grid schedule-stats" style={{marginBottom:18}}>
    ...
    {data.unscheduled_count>0&&<section className="admin-panel schedule-proposal" style={{marginBottom:16}}><div className="admin-panel__top"><span>1 · LÅT CUPNAVI PLANERA</span><strong>INGET SPARAS ÄN</strong></div><h3>{data.unscheduled_count?`Placera ${data.unscheduled_count} oschemalagda matcher`:"Alla matcher har redan en tid"}</h3><p>CupNavi räknar fram ett förslag utifrån planernas öppettider, matchlängd, pauser och lagvila. Befintliga matchtider behålls.</p><div className="admin-form-footer"><span>{proposal?`${proposal.placed_count} matcher fick en tid${proposal.unresolved_count?` · ${proposal.unresolved_count} behöver lösas manuellt`:""}. Granska listan innan du sparar.`:data.unscheduled_count?"Tryck på knappen för att skapa ett granskningsbart förslag.":"Du kan kontrollera eller justera matcherna nedan."}</span><div className="admin-team-actions"><button type="button" onClick={()=>void createProposal()} disabled={busy||data.unscheduled_count===0}>{busy?"Räknar…":proposal?"Räkna om förslaget":"Skapa schemaförslag"}</button>{proposal&&proposal.placed_count>0&&<button type="button" onClick={()=>void applyProposal()} disabled={busy}>Spara det granskade schemat</button>}</div></div>{proposal&&<><h3 className="schedule-review-title">2 · Granska förslaget</h3><div className="admin-dashboard-grid" style={{marginTop:16}}><article className="admin-panel"><strong>{proposal.placed_count}</strong><small>matcher placerade</small></article><article className="admin-panel"><strong>{proposal.unresolved_count}</strong><small>behöver hjälp</small></article><article className="admin-panel"><strong>{proposal.quality.minimum_observed_rest_minutes??"–"}</strong><small>minsta lagvila, min</small></article></div><div className="admin-team-list" style={{marginTop:16}}>{proposal.placements.map(item=>{const match=matchById.get(item.match_id);return <article key={`proposal-${item.match_id}`}><div><strong>{match?`${match.home_label} – ${match.away_label}`:`Match ${item.match_id}`}</strong><small>{item.scheduled_start.replace("T"," ")} · Plan {item.pitch_number}</small></div><strong>FÖRSLAG</strong></article>})}{proposal.unresolved.map(item=>{const match=matchById.get(item.match_id);return <article key={`unresolved-${item.match_id}`}><div><strong>{match?`${match.home_label} – ${match.away_label}`:`Match ${item.match_id}`}</strong><small>{unresolvedLabel(item.reason)}</small></div><strong>MANUELLT</strong></article>})}</div></>}</section>}
    {data.unscheduled_count===0&&analysis.ok&&<section className="schedule-ready-card"><span className="schedule-ready-card__check">✓</span><div><small>{data.schedule_dirty?"DIN KONTROLL ÅTERSTÅR":"SCHEMAT ÄR GODKÄNT"}</small><h3>Alla {data.match_count} matcher har tid och plan</h3><p>{data.schedule_dirty?"Kontrollera avspark och plan i listan. Godkänn sedan schemat mot de bekräftade plantiderna.":"Schemat är kontrollerat mot matchregler och bekräftade plantider."}</p></div>{data.schedule_dirty?<button type="button" className="schedule-confirm-action" disabled={busy} onClick={()=>void confirmSchedule()}>{busy?"Kontrollerar…":"Godkänn schemat →"}</button>:<a href="#publish">Fortsätt till publicering →</a>}</section>}
    {analysis.ok?<div className="admin-code-placeholder schedule-rule-summary" style={{marginBottom:16}}><b>Regler som kontrollerats</b><span>{analysis.match_duration_minutes} min match · {analysis.pitch_break_minutes} min planpaus · minst {analysis.minimum_team_rest_minutes} min lagvila</span></div>:<section className="admin-panel" style={{marginBottom:16}}><div className="admin-panel__top"><span>SCHEMAKONTROLL</span><strong>{analysis.error_count} FEL · {analysis.warning_count} VARNINGAR</strong></div><h3>Åtgärda innan publicering</h3><div className="admin-team-list">{analysis.conflicts.map((conflict,index)=><article key={`${conflict.type}-${conflict.match_ids.join("-")}-${index}`}><div><strong>{conflict.severity==="error"?"STOPP":"VARNING"} · {conflict.message}</strong><small>{conflictMatches(conflict)}</small></div></article>)}</div></section>}
    {proposal&&proposal.capacity.suggestions.length>0&&<section className="admin-panel schedule-capacity-help" style={{marginBottom:16}}><div className="admin-panel__top"><span>VARFÖR SAKNAS TIDER?</span><strong>{proposal.unresolved_count} MATCHER</strong></div><h3>Hela matchprogrammet fick inte plats</h3><p>{proposal.capacity.confirmed_window_count} bekräftade öppettider ger {proposal.capacity.slot_count} möjliga matchstarter. {proposal.capacity.required_count} oschemalagda matcher behöver placeras.</p><div className="admin-team-list">{proposal.capacity.suggestions.map((suggestion,index)=><article key={`capacity-${index}`}><div><strong>Förslag {index+1}</strong><small>{suggestion}</small></div></article>)}</div><div className="admin-form-footer"><span>Ändra kapaciteten och räkna sedan om förslaget.</span><a href="#venues">Öppna Planer & tider →</a></div></section>}
    {data.schedule_dirty&&<div className="schedule-save-state"><b>Sparat men inte godkänt</b><span>Kontrollera matcherna och använd knappen ”Godkänn schemat” ovan.</span></div>}
    <div className="schedule-list-head"><div><small>MATCHLISTA</small><h3>Kontrollera {data.match_count} matcher</h3></div><div className="admin-team-actions schedule-filters"><button type="button" className={filter==="all"?"is-active":""} aria-pressed={filter==="all"} onClick={()=>setFilter("all")}>Alla <span>{data.match_count}</span></button><button type="button" className={filter==="scheduled"?"is-active":""} aria-pressed={filter==="scheduled"} onClick={()=>setFilter("scheduled")}>Med tid <span>{data.scheduled_count}</span></button><button type="button" className={filter==="unscheduled"?"is-active":""} aria-pressed={filter==="unscheduled"} onClick={()=>setFilter("unscheduled")}>Saknar tid <span>{data.unscheduled_count}</span></button></div></div>
